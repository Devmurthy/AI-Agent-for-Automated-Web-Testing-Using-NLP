// Store last generated report URL
...
function displayResults(data) {
    ...
    // Track latest report URL for header button
    if (data.reportUrl) {
    }

    // PDF report link inside results
    if (data.reportUrl) {
        html += `
            <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid rgba(0,0,0,0.08);">
                ...
                <a href="${data.reportUrl}" target="_blank" rel="noopener" 
                   class="btn btn-primary" 
                   style="margin-top: 0.75rem; display: inline-block; text-decoration: none; padding: 10px 18px; font-size: 0.95rem;">
                    <i class="fas fa-download"></i> Download PDF Report
                </a>
                <p style="margin-top: 0.5rem; font-size: 0.85rem; color: #666;">
                    Contains test instruction, what was checked, validations, and performance details.
                </p>
            </div>
        `;
    } else if (data.reportError) {
        html += `
            <div style="margin-top: 1.5rem; color: #dc3545; font-size: 0.9rem;">
                <i class="fas fa-exclamation-triangle"></i> ${data.reportError}
            </div>
        `;
    }
